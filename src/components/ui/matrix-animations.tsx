...
export function MatrixRain({ density = 8, className = "" }: MatrixRainProps) {
  ...
  const generateColumn = useCallback(
    (baseId: number, x: number): MatrixColumn => {
      ...
      const uniqueId =
        baseId * 100000 + Date.now() + Math.floor(Math.random() * 10000);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < density; i++) {
      ...
      const uniqueId =
        i * 100000 + Date.now() + Math.floor(Math.random() * 10000);
    ...
    <div className="absolute inset-0 pointer-events-none opacity-30">
      <svg className="w-full h-full">
        ...
        {points.map((point, index) => {
          const nextRight = points.find(
            (p) => p.x === point.x + 100 / (gridSize - 1) && p.y === point.y,
          );
          const nextDown = points.find(
            (p) => p.x === point.x && p.y === point.y + 100 / (gridSize - 1),
          );
